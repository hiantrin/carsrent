...
const Signagence = () => {
	...
	const [agencenamerr, setagencenamerr] = useState("a");
	...
	const checkerr = async (response) => {
		...
		if (response !== "success")
		{
			if (await response.search("agencename") !== -1)
			{
				setagencenamerr(response);
				setemailrr("a");
				setpasswordrr("a");
				setrepasswordrr("a");
				return false ;
	...
	const checkpass = async () => {
		await Axios.post("http://localhost:3001/sign/Agence/up", {
			...
			if (g === true)
			{
				...
				window.location.pathname = '/Agence/Register';
			}
		}).catch((err) => {
	...
	const checkpassg = async () => {
		await Axios.post("http://localhost:3001/sign/Agence/in", {
			email: semail,
			password: spassword,
		}).then( async (response) => {
    ...
    return (
            ...
            <div className={slide ? 'absolute -top-16 flex-col justify-center items-center mt-32 div-show lg:relative lg:right-32 xl:right-0 div-smo' : 'absolute -top-16 lg:top-0 flex-col justify-center items-center mt-32 div-opacity lg:relative lg:right-32 xl:right-0 div-sm' }>
				...
				<div className="mt-0 lg:mt-6 flex flex-col items-center ">
					<h1 className={(agencenamerr !== "a") ? 'text-red-500 font-bold text-xs' : 'opacity-0'}>{agencenamerr}</h1>
           			<div className={(agencenamerr === "a") ? 'w-60 lg:w-80 h-12 lg:h-16 bg-gray-100  rounded-full mx-0 flex items-center' : 'w-60 lg:w-80 h-12 lg:h-16 bg-gray-100  rounded-full mx-0 flex items-center border-4 border-red-500'}>
						...
						<input type="text" className="font-bold w-40 lg:w-60 h-8 lg:h-10 bg-gray-100 focus:outline-none px-2  placeholder-gray-500" placeholder="Agence" onChange={(event) => {setagencename(event.target.value)}}></input>
                	</div>
				</div>
				<div className="mt-0 lg:mt-6 flex flex-col items-center">
